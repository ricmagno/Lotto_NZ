...
from load_data_bullseye import *
from constants_bullseye import *
...
if sort_balls:
    dft = df.astype(str)
    df['Number'] = (dft['1'] + dft['2'] + dft['3']+dft['4']+ dft['5'] + dft['6']).astype(int)
...
if mode_prediction:
    ...
    for i in balls:
        ...
        number += str(int(round(float(prediction[j]),0)))
        j = j+1
    print('\n=================================================')

    c = db.cursor()
    if sorted:
        number = round(float(prediction[0]),2)
        prediction = ['1', '2', '3', '4', '5' , '6'] 
        j = 0
        for i in str(int(round(number,0))):
            prediction[j] = int(i)
            j += 1

    c.execute("insert into bullseye values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
              [int(df['Draw'].max()+1),
